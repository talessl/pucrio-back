...
class RevogarTokenUseCase:
    ...
    def executar_revogacao(self, paciente_logado_id: int, token_id: int):
        ...
        historico = self.historico_repository.buscar_historico_por_id(token.historico_id)
        if not historico or historico.paciente_id != paciente_logado_id:
            raise ValueError("Acesso negado: Você não tem permissão para revogar este link.")
